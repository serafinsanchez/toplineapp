...
const HoverButton = React.forwardRef<HTMLButtonElement, HoverButtonProps>(
  ({ className, children, ...props }, ref) => {
    ...
    // Extract gradient colors from className
    const fromColor = className?.includes('from-') ? 
      className.match(/from-([a-z0-9-]+)/)?.[1] : 'blue-400'
    const toColor = className?.includes('to-') ? 
      className.match(/to-([a-z0-9-]+)/)?.[1] : 'indigo-600'
